...
namespace Forecast.Tests.Clients
{
    public class GoogleWeatherDataClientTests
    {
        ...
        public async Task CorrectRequsetUrl()
        {
            ...
            Assert.Contains("location.latitude=10", capturedRequest!.RequestUri!.ToString());
            Assert.Contains("location.longitude=20", capturedRequest.RequestUri.ToString());
            Assert.Contains("key=somekey", capturedRequest.RequestUri.ToString());
        }
    }
}
